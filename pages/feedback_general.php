...
<section class="section glass-card active" style="padding:2rem">
  <h1 class="accent-text" style="font-size:2rem; font-weight:800; margin-bottom:1rem">General Feedback</h1>
  <p class="muted" style="margin-bottom:1rem">Share your thoughts and suggestions.</p>
  <form id="generalForm" enctype="multipart/form-data" style="display:flex; flex-direction:column; gap:1rem">
...
<script>
  document.getElementById('generalForm').addEventListener('submit', async (e) => {
    ...
    try{
      ...
      if (window.FitnessAPI?.notifications) FitnessAPI.notifications.success('Feedback submitted!');
      e.target.reset();
    }catch(err){
